...
LANGS = {
    "kor": {
        ...
        "meta": "대상: 전 지점 담당자 · 관리자  |  언어: 한국어  |  2026년 7월 기준",
        ...
        "sections": [
            ...
            {"title": "5. 입고 처리",
             "body": [
                ("p", "본사 또는 외부에서 양식을 수령했을 때 처리합니다. 좌측 메뉴 '입고'를 클릭하면 '개별 입고' / '일괄 입고' 두 개의 탭이 표시됩니다."),
                ("h3", "개별 입고"),
                ("step", (1, "지점(관리자만 선택, 지점 계정은 자동 설정), 양식 종류, 수량, 날짜, 비고 입력")),
                ("step", (2, "'입고 등록' 버튼 클릭")),
                ("h3", "일괄 입고"),
                ("step", (1, "관리자는 지점 선택 드롭다운에서 조회할 지점 선택 (지점 계정은 본인 지점이 자동 표시됨)")),
                ("step", (2, "입고할 양식들을 체크박스로 선택하고, 각 항목의 입고 수량·비고 입력")),
                ("step", (3, "입고일자 선택 후 '선택 항목 입고' 버튼 클릭")),
                ("info", "입고 후에도 최소기준 이하이면 자동으로 알림이 생성됩니다."),
             ]},
            {"title": "6. 출고 처리",
             "body": [
                ("p", "양식을 실제로 사용하거나 외부로 내보낼 때 처리합니다. 좌측 메뉴 '출고'를 클릭하면 '개별 출고' / '일괄 출고' 두 개의 탭이 표시됩니다."),
                ("h3", "개별 출고"),
                ("step", (1, "지점, 양식 종류, 수량, 날짜, 비고 입력")),
                ("step", (2, "'출고 등록' 버튼 클릭")),
                ("warn", "현재 재고보다 많은 수량은 출고할 수 없습니다. 출고 후 최소기준 이하이면 자동 알림이 발송됩니다."),
                ("h3", "일괄 출고"),
                ("step", (1, "관리자는 지점 선택 드롭다운에서 조회할 지점 선택 (지점 계정은 본인 지점이 자동 표시됨)")),
                ("step", (2, "출고할 양식들을 체크박스로 선택하고, 각 항목의 출고 수량·비고 입력 (0이면 건너뜀)")),
                ("step", (3, "출고 기간(월) 선택 후 '선택 항목 출고' 버튼 클릭")),
    ...
    "eng": {
        ...
        "meta": "For: All Branch Staff & Administrators  |  Language: English  |  July 2026",
        ...
        "sections": [
            ...
            {"title": "5. Inbound Processing",
             "body": [
                ("p", "Used when forms are received from HQ or an external source. Click Inbound in the left menu to see two tabs: Individual Inbound and Bulk Inbound."),
                ("h3", "Individual Inbound"),
                ("step", (1, "Enter branch (admin only; branch accounts are set automatically), form type, quantity, date, notes")),
                ("step", (2, "Click Register Inbound")),
                ("h3", "Bulk Inbound"),
                ("step", (1, "Admins select the branch to view from the branch dropdown (branch accounts show their own branch automatically)")),
                ("step", (2, "Check the forms to receive and enter the inbound quantity/notes for each item")),
                ("step", (3, "Select the inbound date, then click Process Selected")),
            ...
            {"title": "6. Outbound Processing",
             "body": [
                ("p", "Used when forms are actually used or sent outside. Click Outbound in the left menu to see two tabs: Individual Outbound and Bulk Outbound."),
                ("h3", "Individual Outbound"),
                ("step", (1, "Enter branch, form type, quantity, date, notes")),
                ("step", (2, "Click Register Outbound")),
                ("warn", "Cannot outbound more than current stock. An alert is sent automatically if stock falls below minimum threshold."),
                ("h3", "Bulk Outbound"),
                ("step", (1, "Admins select the branch to view from the branch dropdown (branch accounts show their own branch automatically)")),
                ("step", (2, "Check the forms to ship and enter the outbound quantity/notes for each item (0 to skip)")),
                ("step", (3, "Select the outbound period (month), then click Process Selected")),
            ...
            {"title": "7. Branch-to-Branch Transfer",
             "body": [
            ...
            {"title": "8. Transaction History & Reports",
             "body": [
            ...
            {"title": "9. Catalog (Transport Items) Request",
             "body": [
    ...
    "jpn": {
        ...
        "meta": "対象：全支店スタッフ・管理者  |  言語：日本語  |  2026年7月現在",
        ...
        "sections": [
            ...
            {"title": "5. 入庫処理",
             "body": [
                ("p", "本社または外部から様式を受け取った際に処理します。左メニュー「入庫」をクリックすると「個別入庫」「一括入庫」の2つのタブが表示されます。"),
                ("h3", "個別入庫"),
                ("step", (1, "支店（管理者のみ選択、支店アカウントは自動設定）、様式種類、数量、日付、備考を入力")),
                ("step", (2, "「入庫登録」ボタンをクリック")),
                ("h3", "一括入庫"),
                ("step", (1, "管理者は支店選択ドロップダウンで対象支店を選択（支店アカウントは自支店が自動表示）")),
                ("step", (2, "入庫する様式にチェックを入れ、各項目の入庫数量・備考を入力")),
                ("step", (3, "入庫日を選択し「選択品目入庫」ボタンをクリック")),
                ("info", "入庫後も最低基準以下の場合、自動的にアラートが生成されます。"),
             ]},
            {"title": "6. 出庫処理",
             "body": [
                ("p", "様式を実際に使用または外部へ送る際に処理します。左メニュー「出庫」をクリックすると「個別出庫」「一括出庫」の2つのタブが表示されます。"),
                ("h3", "個別出庫"),
                ("step", (1, "支店、様式種類、数量、日付、備考を入力")),
                ("step", (2, "「出庫登録」ボタンをクリック")),
                ("warn", "現在庫より多い数量は出庫できません。出庫後に最低基準以下になると自動通知が送信されます。"),
                ("h3", "一括出庫"),
                ("step", (1, "管理者は支店選択ドロップダウンで対象支店を選択（支店アカウントは自支店が自動表示）")),
                ("step", (2, "出庫する様式にチェックを入れ、各項目の出庫数量・備考を入力（0は無視）")),
                ("step", (3, "出庫期間（月）を選択し「選択品目出庫」ボタンをクリック")),
    ...
    "chn": {
        ...
        "title": "用户手册",
        "sub": "易斯达航空 物流·备品管理系统",
        "meta": "适用对象：全部网点工作人员·管理员  |  语言：中文（简体）|  2026年7月版",
        ...
        "sections": [
            ...
            {"title": "5. 入库处理",
             "body": [
                ("p", "在从总部或外部收到表单时进行处理。点击左侧菜单「入库」后会显示「单项入库」「批量入库」两个标签页。"),
                ("h3", "单项入库"),
                ("step", (1, "输入网点（仅管理员可选择，网点账户自动设定）、表单种类、数量、日期、备注")),
                ("step", (2, "点击「入库登记」按钮")),
                ("h3", "批量入库"),
                ("step", (1, "管理员可在网点选择下拉菜单中选择要查看的网点（网点账户自动显示本网点）")),
                ("step", (2, "勾选要入库的表单，输入各项目的入库数量·备注")),
                ("step", (3, "选择入库日期后点击「处理选中项」")),
                ("info", "入库后若仍低于最低标准，系统将自动生成警报通知。"),
             ]},
            {"title": "6. 出库处理",
             "body": [
                ("p", "在实际使用表单或送往外部时进行处理。点击左侧菜单「出库」后会显示「单项出库」「批量出库」两个标签页。"),
                ("h3", "单项出库"),
                ("step", (1, "输入网点、表单种类、数量、日期、备注")),
                ("step", (2, "点击「出库登记」按钮")),
                ("warn", "出库数量不能超过当前库存。出库后若库存降至最低标准以下，将自动发送通知。"),
                ("h3", "批量出库"),
                ("step", (1, "管理员可在网点选择下拉菜单中选择要查看的网点（网点账户自动显示本网点）")),
                ("step", (2, "勾选要出库的表单，输入各项目的出库数量·备注（0则跳过）")),
                ("step", (3, "选择出库期间（月）后点击「处理选中项」")),
            ...
            {"title": "9. 目录（运输物品）申请",
             "body": [
                ...
                ("h3", "在购物车中确认．调整"),
                ("p", "• 部分选择申请 — 可用购物车内各项目前的复选框，只选择部分项目提交申请。取消勾选的项目会留在购物车中，之后可以继续申请。"),
                ("p", "• 必填申请原因 — X-Banner、立牌等定制项目，必须填写申请原因才能加入购物车。"),
                ("p", "• 自定义图片预览 — 含有已编辑自定义图片的项目会显示「自定义图片」标签，点击即可放大确认。"),
             ]},
            {"title": "10. 表单耗材申请",
             "body": [
                ("step", (1, "点击左侧菜单「表单申请」")),
    ...
    "twn": {
        ...
        "title": "用戶手冊",
        "sub": "易斯達航空 物流·備品管理系統",
        "meta": "適用對象：全部據點工作人員·管理員  |  語言：中文（繁體）|  2026年7月版",
        ...
        "sections": [
            ...
            {"title": "5. 入庫處理",
             "body": [
                ("p", "在從總部或外部收到表單時進行處理。點擊左側選單「入庫」後會顯示「單項入庫」「批量入庫」兩個標籤頁。"),
                ("h3", "單項入庫"),
                ("step", (1, "輸入據點（僅管理員可選擇，據點帳戶自動設定）、表單種類、數量、日期、備註")),
                ("step", (2, "點擊「入庫登記」按鈕")),
                ("h3", "批量入庫"),
                ("step", (1, "管理員可在據點選擇下拉選單中選擇要查看的據點（據點帳戶自動顯示本據點）")),
                ("step", (2, "勾選要入庫的表單，輸入各項目的入庫數量·備註")),
                ("step", (3, "選擇入庫日期後點擊「處理選中項」")),
                ("info", "入庫後若仍低於最低標準，系統將自動產生警報通知。"),
             ]},
            {"title": "6. 出庫處理",
             "body": [
                ("p", "在實際使用表單或送往外部時進行處理。點擊左側選單「出庫」後會顯示「單項出庫」「批量出庫」兩個標籤頁。"),
                ("h3", "單項出庫"),
                ("step", (1, "輸入據點、表單種類、數量、日期、備註")),
                ("step", (2, "點擊「出庫登記」按鈕")),
                ("warn", "出庫數量不能超過當前庫存。出庫後若庫存降至最低標準以下，將自動發送通知。"),
                ("h3", "批量出庫"),
                ("step", (1, "管理員可在據點選擇下拉選單中選擇要查看的據點（據點帳戶自動顯示本據點）")),
                ("step", (2, "勾選要出庫的表單，輸入各項目的出庫數量·備註（0則略過）")),
                ("step", (3, "選擇出庫期間（月）後點擊「處理選中項」")),
